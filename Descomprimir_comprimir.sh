...
function listarHerramientas(){
    ...
    comandos=("bzip2" "gzip" "tar" "zip" "7z" "unzip" "gunzip" "unrar" "rar" "bzcat")
            ...
            1)
                if [ "${Dbzip2}" -eq 1 ];then
                   errorLog 12 "bzip2"
                    ...
                    if bzip2 -9 -v -c -z -f "${ruta_comprimir}"/* > "${salvar_comprimida}/${nombre_guardado}.bz2" ;then
...
2)
    while [ "${v}" -eq 0 ];do
    clear
    echo -e "${Blanco}Ha seleccionado formato ${Purpura}'${reset}${S_Rojo}Descompresor${reset}${Purpura}'${reset}${NAmarillo}.${reset}"
    ...
    clear
    echo -e "${S_Purpura}¿Qué escoges?${reset}"
            ...
            1)
                if [ "${D7z}" -eq 1 ];then
                   errorLog 12 "7z"
                    ...
                    clear
                    if bzcat -dkf "${ruta_descomprimir}/${fichero_descomprimir}" > "${ruta_salvado}/${nombre_guardado}" 2>/dev/null;then
